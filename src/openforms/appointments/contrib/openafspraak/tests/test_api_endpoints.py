...
from .utils import MockConfigMixin, OPENAFSPRAAK_BASE_URL
...
class ResponseCodeTests(MockConfigMixin, TestCase):
    ...
    def test_internal_server_error(self, m):
        m.get(f"{OPENAFSPRAAK_BASE_URL}/products", status_code=500, text="Internal Server Error")

        with self.assertRaises(OpenAfspraakException) as context:
            plugin.get_available_products()
            self.assertEqual("Server error (HTTP 500): Internal Server Error", str(context.exception))
